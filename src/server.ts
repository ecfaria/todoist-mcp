...
async function main() {
    ...
    'todoist_create_task',
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          project_id: { type: 'string', description: 'Optional project ID to add the task to' },
          section_id: { type: 'string', description: 'Optional section ID within the project' },
          due_date: { type: 'string', description: 'Due date in natural language (e.g., "tomorrow", "next Monday") or YYYY-MM-DD format' },
          priority: { type: 'number', description: 'Priority level: 1 (normal), 2 (medium), 3 (high), 4 (urgent)', enum: [1, 2, 3, 4] },
          ...
          parent_task_name: { type: 'string', description: 'Parent task name to search for (alternative to parent_id). The tool will search for tasks matching this name.' },
        },
        required: ['content'],
      } as any,
    },
    async (args: any) => {
      logger.debug('Tool called: todoist_create_task');
      return await createTask(client, args) as any;
    }
  );
    ...
    'todoist_list_tasks',
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          limit: { type: 'number', description: 'Maximum number of tasks to return (default: 50, max: 200)', default: 50 },
        },
      } as any,
    },
    async (args: any) => {
      logger.debug('Tool called: todoist_list_tasks');
      return await listTasks(client, args) as any;
    }
  );
  ...
  server.registerTool(
    ...
    async (args: any) => {
      logger.debug('Tool called: todoist_get_task');
      return await getTask(client, args) as any;
    }
  );
  ...
  server.registerTool(
    'todoist_update_task',
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          due_date: { type: 'string', description: 'New due date (natural language or YYYY-MM-DD)' },
          priority: { type: 'number', description: 'New priority level (1-4)', enum: [1, 2, 3, 4] },
    ...
    async (args: any) => {
      logger.debug('Tool called: todoist_update_task');
      return await updateTask(client, args) as any;
    }
  );
  ...
  server.registerTool(
    ...
    async (args: any) => {
      logger.debug('Tool called: todoist_complete_task');
      return await completeTask(client, args) as any;
    }
  );
  ...
  server.registerTool(
    ...
    async (args: any) => {
      logger.debug('Tool called: todoist_list_projects');
      return await listProjects(client, args) as any;
    }
  );
  ...
  server.registerTool(
    'todoist_search_tasks',
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'Text to search for in task content and description' },
          limit: { type: 'number', description: 'Maximum number of tasks to return (default: 50, max: 200)', default: 50 },
        },
        required: ['query'],
      } as any,
    ...
    async (args: any) => {
      logger.debug('Tool called: todoist_search_tasks');
      return await searchTasks(client, args) as any;
    }
  );
